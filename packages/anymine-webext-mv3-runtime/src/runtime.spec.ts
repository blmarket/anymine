...
import { WebExtMv3Runtime } from "./runtime";

describe('webext mv3 runtime', () => {
  let runtime: WebExtMv3Runtime;

  beforeAll(async () => {
    runtime = await WebExtMv3Runtime.create();
  });
  ...
  it('allows read and write arbitrary cookies', async () => {
    await runtime.evaluate(`chrome.cookies.set({ url: "https://example.com", name: "test", value: "test" }).then(() => '');`);
